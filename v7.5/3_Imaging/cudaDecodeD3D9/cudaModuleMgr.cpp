...
CUresult modGetCudaFunction(sCtxModule *pCtx, const char *func_name,    CUfunction *fpCudaKernel)
{
    ...
    printf(">> modGetCudaFunction<%s>\n", pCtx->mModuleName.c_str());
    printf("   CUDA Kernel Function = <%s, 0x%08x>\n", func_name, (size_t)(pCtx->pCudaKernels_[idx].fpCuda));
...
CUresult modGetCudaDevicePtr(sCtxModule *pCtx, const char *address_name, CUdeviceptr *pGlobalMem)
{
    ...
    printf(">> modGetCudaDevicePtr<%s>\n", pCtx->mModuleName.c_str());
    printf("   CUDA Device Memory <%s, 0x%08x>\n", address_name, (unsigned int)(pCtx->pGlobalMem_[idx].devicePtr));
...
CUresult modGetTexRef(sCtxModule *pCtx, const char *texref_name,  CUtexref    *pTexRef)
{
    ...
    printf(">> modGetTexRef<%s>\n", pCtx->mModuleName.c_str());
    printf("   CUDA TextureReference <%s, 0x%08x>\n", texref_name, (size_t)(pCtx->pTexRef_[idx].texRef));
