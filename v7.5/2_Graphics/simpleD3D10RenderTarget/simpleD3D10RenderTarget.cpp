...
struct Color
{
    ...
    ID3D10RenderTargetView     *pBufferRTV; //The Render target view on the color buffer
    ID3D10ShaderResourceView   *pBufferSRV; //The shader resource view on the color buffer
...
static const char g_displayEffectSrc[] =
    ...
    "{\n" \
    "    Fragment f;\n" \
    "    f.Tex = float2( 1.f*((vertexId == 1) || (vertexId == 3)), 1.f*( vertexId >= 2)); \n"\
    "    \n" \
    "    uint2 cellId = uint2(instanceId % g_vGrid.x, instanceId / g_vGrid.x);\n" \
    "    f.Pos = float4( g_vGridSize.xy*cellId + 0.5f*(g_vGridSize.xy - g_vGridSize.zw) + f.Tex * g_vGridSize.zw, 0, 1);\n" \
    "    f.Pos.xy = (f.Pos.xy*2.f - 1.f);\n" \
    "    \n" \
    "    f.Col = float4( ((g_vGrid.x-1.f) - cellId.x) / (g_vGrid.x-1.f), (cellId.x + (g_vGrid.y-1.f) - cellId.y) / (g_vGrid.x+g_vGrid.y-1.f), cellId.y / (g_vGrid.y-1.f), 1.f);\n" \
    "    f.Col *= float4( 0.5 + 0.5*sin(g_Time), 0.5 + 0.5*sin(g_Time)*cos(g_Time), 0.5 + 0.5*cos(g_Time), 1.f);\n" \
    "    return f;\n" \
    "}\n" \
    "\n" \
...
static const char g_compositeEffectSrc[] =
    ...
    "{\n" \
    "    Fragment f;\n" \
    "    f.Tex = float3( 1.f*((vertexId == 1) || (vertexId == 3)), 1.f*( vertexId >= 2), 0.f); \n"\
    "    \n" \
    ...
    "        float red = ( value >= f.uv.y ? (0.5f * f.uv.y / value) + 0.5f : 0.f ); \n" \
...
extern "C" void createHistogramTex(unsigned int *histogram, unsigned int width, unsigned int height, cudaArray *colorArray);
