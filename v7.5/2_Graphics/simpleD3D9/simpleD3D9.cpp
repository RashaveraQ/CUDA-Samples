...
bool checkHW(char *name, char *gpuType, int dev)
{
    ...
    STRCPY(name, NAME_LEN, deviceProp.name);
...
int findGraphicsGPU(char *name)
{
    ...
    char firstGraphicsName[NAME_LEN], temp[NAME_LEN];
    ...
    if (error_id != cudaSuccess)
    {
        ...
        printf("%s returned ERROR, exiting...\n", sSDKsample);
        exit(EXIT_FAILURE);
    }
    ...
    for (int dev = 0; dev < deviceCount; ++dev)
    {
        ...
        if (bGraphics)
        {
            if (!bFoundGraphics)
            {
                STRCPY(firstGraphicsName, NAME_LEN, temp);
            }
    ...
    if (nGraphicsGPU)
    {
        STRCPY(name, NAME_LEN, firstGraphicsName);
    }
    else
    {
        STRCPY(name, NAME_LEN, "this hardware");
    }
...
int main(int argc, char **argv)
{
    ...
    if (!findGraphicsGPU(device_name))
    {
        ...
        printf("> %s sample finished, exiting...\n", sSDKsample);
        exit(EXIT_SUCCESS);
    }
...
void runTest(int argc, char **argv, char *ref_file)
{
    ...
    HWND hWnd = CreateWindow(wc.lpszClassName, "CUDA/D3D9 simpleD3D9",
...
HRESULT InitVertexBuffer()
{
    ...
    if (FAILED(g_pD3DDevice->CreateVertexBuffer(g_NumVertices * sizeof(CUSTOMVERTEX),
