...
bool checkHW(char *name, char *gpuType, int dev)
{
    ...
    if (!strnicmp(deviceProp.name, gpuType, strlen(gpuType)))
    {
        return true;
    }
    else
...
HRESULT InitVertexBuffer()
{
    ...
    if (FAILED(g_pD3DDevice->CreateVertexBuffer(DS * sizeof(Vertex),
...
HRESULT InitPointTexture()
{
    ...
    if (FAILED(g_pD3DDevice->CreateTexture(width, height, 0, D3DUSAGE_AUTOGENMIPMAP | D3DUSAGE_DYNAMIC, 
...
HRESULT InitD3D9RenderState()
{
    ...
    D3DXMatrixOrthoOffCenterLH(&matProj, 0, 1, 1, 0, 0, 1);
...
HRESULT InitCUFFT()
{
    ...
    cufftSetCompatibilityMode(g_planr2c, CUFFT_COMPATIBILITY_FFTW_PADDING);
    cufftSetCompatibilityMode(g_planc2r, CUFFT_COMPATIBILITY_FFTW_PADDING);
...
int main(int argc, char **argv)
{
    ...
    hWnd = CreateWindow("fluidsD3D9", "CUDA/D3D9 Stable Fluids",
                        WS_OVERLAPPEDWINDOW, 100, 100, wWidth + 2*xBorder, wHeight+ 2*yBorder+yCaption,
                        NULL, NULL, wc.hInstance, NULL);

    if (SUCCEEDED(InitD3D9(hWnd)) &&
        SUCCEEDED(InitCUDA()) &&
        SUCCEEDED(InitD3D9RenderState()) &&
        SUCCEEDED(InitCUFFT()) &&
        SUCCEEDED(InitVertexBuffer()) &&
        SUCCEEDED(InitPointTexture()))
    {
        ...
        while (msg.message!=WM_QUIT)
        {
            ...
            else
            {
                ...
                if (ref_file)
                {
                    ...
                    g_bPassed = CheckRenderD3D9::PPMvsPPM(cur_image_path, ref_file, argv[0], MAX_EPSILON, 0.30f);
