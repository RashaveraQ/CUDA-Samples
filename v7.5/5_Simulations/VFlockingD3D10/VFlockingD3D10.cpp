...
extern "C"
void cuda_simulate(float2 *newPos, float2 *curPos, uint numBirds, bool *d_hasproxy, bool *d_neighbors, bool *d_leftgoals,
                   bool *d_rightgoals, uint2 *d_pairs, uint3 *d_triples, Params *m_params) ;
...
bool findCUDADevice()
{
    ...
    char firstGraphicsName[256], devname[256];
...
int main(int argc, char *argv[])
{
    ...
    HWND hWnd = CreateWindow(wc.lpszClassName, "VFlocking",
...
void initForCUDA(uint numBirds)
{
    ...
    checkCudaErrors(cudaMemcpy(d_triples, triples, nBirds * (nBirds - 1) * (nBirds - 2) * sizeof(uint3) / 6,
...
bool compare(WingTip &t1, WingTip &t2)
{
